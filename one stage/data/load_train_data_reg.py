...
class TrainData():
    ...
    def __getitem__(self, item):
        ...
        data_points, teeth_landmarks, seg_tooth_label = read_data(file_path)
        mask1 = (seg_tooth_label <= 8) &  (seg_tooth_label >= 1)
        mask2 = seg_tooth_label >= 9

        seg_label = seg_tooth_label.copy()
        seg_label[mask1] = 1
        seg_label[mask2] = 2
        #Decentering data
        ...
        heat_map, offest_map, class_map, mask = get_hotmap(data_points, teeth_landmarks, seg_tooth_label, knums=cfg.neark)
        ...
        return data_points, seg_label, heat_map, offest_map, class_map, mask, teeth_landmarks


def get_hotmap(points, teeth_landmarks, seg_tooth_label, sigma=0.7, knums=20):  #8192*1
#def get_hotmap(points, teeth_landmarks, sigma=1, knums=40):#8192*2
    num_pts = points.shape[0]
    num_classes = cfg.landmarks_class

    # 使用输入点的实际规模初始化
    heat_map = np.zeros((num_pts, num_classes), np.float32)
    offset_map = np.zeros((num_pts, num_classes, 3), np.float32)
    mask = np.zeros((num_pts, num_classes), np.uint8)
    ...
    for i, clss_ in enumerate(teeth_landmarks):
        ...
        #id = cfg.class_type[i]
        # 5. 填充结果
        #heat_map[final_indw, i] = final_wei
        heat_map[final_indices, i] = final_weights
        offset_map[final_indices, i, :] = final_offv
        mask[final_indices, i] = 1
