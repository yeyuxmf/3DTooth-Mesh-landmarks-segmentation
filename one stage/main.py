...
from model.tooth_landmarks_onet_paper import ToothLandmark
...
def train(args, io):
    ...
    model = ToothLandmark()#
    # model_ema = model
    model_path = "./outputs/land_model_best.pth"
    ...
    for epoch in range(args.epochs):
        ...
        nums =0
        land_nums = 0
        for batch_data, heat_map, offest_map, mask in train_loader:
            ...
            if nums % cfg.LOSSNUMS == 0:
                ...
                print(
                    'epoch %d /%d,epoch %d /%d, total_loss: %.6f, dice_loss: %.6f, wing_Loss: %.6f, const time: %.6f' % (
                        epoch, args.epochs, nums, inter_nums, total_loss, dice_loss, wing_Loss, toc - tic))
                total_loss = 0
                dice_loss = 0
                wing_Loss = 0
        ...
        if epoch+1 >=80 and 0 == (epoch+1)%1:
            ...
            if ap_ar_v > max_ap_ar:
                max_ap_ar = ap_ar_v
                torch.save({'model': model.state_dict(), 'epoch': epoch}, 'outputs/' + 'land_model_best.pth')
    ...
    torch.save({'model': model.state_dict(), 'epoch': epoch}, 'outputs/land_model_final' + "" + '.pth')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_sgd', type=bool, default=True,
                        help='Use SGD')
    parser.add_argument('--lr', type=float, default=1*1e-4, metavar='LR',
