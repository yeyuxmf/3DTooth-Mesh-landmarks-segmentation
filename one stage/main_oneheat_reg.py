...
from data.load_train_data_oneheat_reg import TrainData
from data.load_test_data import TestData
from torch.utils.data import DataLoader
from model.tooth_landmarks_onet_oneheat_reg  import TwoStageToothPipeline
from model.loss import ToothHeatmapLoss
from model.oneheat_loss import OneMultiInstanceMatchLoss,Stage2LocalLandmarkLoss

import config.oneheat_config as cfg
from util import IOStream, cal_acc_seg_land_oneheat_reg
...
def train(args, io):
    ...
    file_path = 'H:/teethMICCAI2022/train_land_onenet_data/trainoneheat/'
    train_loader = DataLoader(TrainData(file_path), num_workers=0,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)

    file_path = "H:/teethMICCAI2022/train_land_onenet_data/testc/"
    ...
    # torch.jit.save(scripted_module, './save_model/seg_land_oneheat_final.pt')
    # model_ = torch.jit.load('./save_model/seg_land_oneheat_final.pt').cuda().eval().float()
    ...
    stage_loss = Stage2LocalLandmarkLoss()
    max_ap_ar = 0
    max_acc = 0
    for epoch in range(args.epochs):
        ...
        #
        #     preheat_map, preoff_map, cls, init_land, init_conf, shifted_pos_all = output["onestage"]
        #     twoheatmap, twooffset_raw, twoseg_mask, final_centers, relative_pos = output["twostage"]
        #
        #     dice_loss_ = heatt_loss(preheat_map, heat_map, mask)
        #     cls_loss_ = heatt_loss(cls, gtcls, gtcls.cuda().bool())
        #
        #     wing_Loss_ = off_loss(preoff_map.float()[mask.expand_as(offest_map)], offest_map.float()[mask.expand_as(offest_map)])
        #
        #     comm_loss, tooth_conf_loss_, tooth_coord_loss_ = match_loss(init_conf, init_land, shifted_pos_all, teeth_landmarks)
        #     loss_s2, hm_loss_s2, off_loss_s2 = stage_loss(final_centers, relative_pos,twoheatmap, twooffset_raw,[teeth_landmarks])
        #
        #     loss = dice_loss_ + wing_Loss_  + comm_loss + loss_s2
        #
        #
        ...
        #
        #     coord_loss += tooth_coord_loss_.item()
        #     tooth_conf_loss += tooth_conf_loss_.item()
        #
        #
        ...
        #         tooth_conf_loss = tooth_conf_loss / cfg.LOSSNUMS
        #
        #         print("lr = ", optimizer.param_groups[0]['lr'])
        #         print("coord_loss = ", coord_loss,  "tooth_conf_loss = ", tooth_conf_loss,"hm_loss = ", hm_loss_s2.item(),  "off_loss = ", off_loss_s2.item())
        #
        #         print(
        ...
        if epoch+1 >=0 and 0 == (epoch+1)%1:
            model.eval()
            fm_cdv, all_metrics, score, cd_all = cal_acc_seg_land_oneheat_reg(model, test_loader)
            ...
            if max_acc < np.mean(np.array(list(all_metrics["AP"].values()))):
                ...
                #torch.save({'model': model.state_dict(), 'epoch': epoch}, 'outputs/' + 'seg_land_oneheat_best.pth')
    ...
    torch.save({'model': model.state_dict(), 'epoch': epoch}, 'outputs/seg_land_oneheat_final' + "" + '.pth')
