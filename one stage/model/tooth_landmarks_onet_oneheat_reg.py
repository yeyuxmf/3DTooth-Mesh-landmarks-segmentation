...
class ToothLandmark(nn.Module):
    ...
    def forward(self, x):
        ...
        heatmap = torch.sigmoid(self.heatmap_head(final_features)).permute(0, 2, 1)
        cls_scores = torch.sigmoid(self.class_t(final_features)).permute(0, 2, 1)
        offset_raw = self.offset_head(final_features).permute(0, 2, 1)
        ...
        # K_edge = idx.shape[-1]
        #
        # heat = heatmap.squeeze(-1)
        # neighbor_heat = torch.gather(heat.unsqueeze(-1).expand(B, N, K_edge), 1, idx)
        # local_max = heat >= neighbor_heat.max(dim=-1)[0]
        #
        # priority_map = torch.where(local_max, 2.0 + heat, 1.0 - heat)
        # _, topk_indices = torch.topk(priority_map, k=self.topk, dim=-1)
        #
        # num_candidates = self.topk
        # topk_idx_3 = topk_indices.unsqueeze(-1).expand(-1, -1, 3)
        # cand_raw_pos = torch.gather(raw_pos, 1, topk_idx_3)
        # cand_offset = torch.gather(offset_raw.detach(), 1, topk_idx_3)
        # shifted_pos_all = cand_raw_pos + cand_offset
        #######################################################################
        #########################################################################################################
        # --- 原来的代码：寻找局部极值与种子点 ---
        heat = heatmap.squeeze(-1)  # (B, N)
        B, N = heat.shape
        K_edge = idx.shape[-1]

        # 使用高级索引获取每个点的 K 个邻域的热力值，判断局部极值
        B_idx_all = torch.arange(B).view(-1, 1, 1).to(heat.device)
        neighbor_heat_all = heat[B_idx_all, idx]  # (B, N, K_edge)
        local_max = heat >= neighbor_heat_all.max(dim=-1)[0]
        ...
        # --- 新增的改进逻辑：带距离约束的局部 KNN 加权中心预测 ---

        # 1. 提取 topK 种子点的坐标 (cand_raw_pos 保留，供后续流程复用)
        B_idx_seed = torch.arange(B).view(-1, 1).to(heat.device)
        cand_raw_pos = raw_pos[B_idx_seed, topk_indices, :]  # (B, num_candidates, 3)

        # 2. 提取种子点的 K 个邻居的索引
        seed_neighbor_idx = idx[B_idx_seed, topk_indices, :]  # (B, num_candidates, K_edge)

        # 3. 提取邻居点的 原始坐标、预测偏移量 和 热力值
        B_idx_neighbor = torch.arange(B).view(-1, 1, 1).to(heat.device)
        neighbor_raw_pos = raw_pos[B_idx_neighbor, seed_neighbor_idx, :]  # (B, num_candidates, K_edge, 3)
        neighbor_offset = offset_raw.detach()[B_idx_neighbor, seed_neighbor_idx, :]  # (B, num_candidates, K_edge, 3)
        neighbor_heat = heat[B_idx_neighbor, seed_neighbor_idx]  # (B, num_candidates, K_edge)

        # 4. 计算这 K 个邻居点各自预测的绝对中心位置
        neighbor_shifted_pos = neighbor_raw_pos + neighbor_offset  # (B, num_candidates, K_edge, 3)

        # 5. 距离约束：计算邻居点与种子点的物理距离，生成 < 1.0mm 的 Mask
        # unsqueeze(2) 是为了让形状匹配: (B, num_candidates, 1, 3) 与 (B, num_candidates, K_edge, 3) 计算距离
        dists_to_seed = torch.norm(neighbor_raw_pos - cand_raw_pos.unsqueeze(2), dim=-1)  # (B, num_candidates, K_edge)
        dist_mask = (dists_to_seed < 1.0).float()  # (B, num_candidates, K_edge)

        # 6. 计算加权权重：(网络预测的置信度 heat) * (距离掩码)
        weights = neighbor_heat * dist_mask  # (B, num_candidates, K_edge)

        # 为了避免除零错误（极大概率至少包含自己，因为自己到自己的距离为0，且heat>0），加上极小值 eps
        weights_sum = weights.sum(dim=-1, keepdim=True).clamp(min=1e-6)  # (B, num_candidates, 1)

        # 7. 归一化并求加权中心 (将权重扩展至 xyz 3个通道进行相乘相加)
        weights_3d = weights.unsqueeze(-1)  # (B, num_candidates, K_edge, 1)
        shifted_pos_all = (neighbor_shifted_pos * weights_3d).sum(dim=2) / weights_sum  # (B, num_candidates, 3)
        #########################################################################################################
        #########################################################################################################

        dist = torch.cdist(shifted_pos_all, raw_pos)
        _, kreg_idx = torch.topk(dist, k=self.k_reg, dim=-1, largest=False)
        kreg_idx_flat = kreg_idx.view(B, -1)

        kreg_idx_flat_3 = kreg_idx_flat.unsqueeze(-1).expand(-1, -1, 3)
        raw_pos_kreg = torch.gather(raw_pos, 1, kreg_idx_flat_3)
        raw_pos_kreg = raw_pos_kreg.view(B, num_candidates, self.k_reg, 3).permute(0, 3, 1, 2)

        centers = shifted_pos_all.transpose(1, 2).unsqueeze(-1)
        rel_pos_kreg = raw_pos_kreg - centers

        D = final_features.shape[1]
        kreg_idx_flat_D = kreg_idx_flat.unsqueeze(1).expand(-1, D, -1)
        final_features_kreg = torch.gather(final_features, 2, kreg_idx_flat_D)
        final_features_kreg = final_features_kreg.view(B, D, num_candidates, self.k_reg)
        lfea = self.mapliner(final_features_kreg)

        pos_embed = self.pos_embed(rel_pos_kreg)
        lfea = torch.cat([lfea, pos_embed], dim=1)
        pfea = self.local_refiner(lfea)

        init_conf = self.predconf(pfea)
        init_land = self.predland1(pfea)
