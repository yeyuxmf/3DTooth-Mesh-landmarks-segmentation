...
class ToothLandmark(nn.Module):
    def __init__(self, classnums=6, k=20, emb_dims=1024):
        ...
        self.edge_conv5 = EdgeConvBlock(self.dim * 4, self.dim * 4, k=k, groups=groups)
        self.edge_conv6 = EdgeConvBlock(self.dim * 4, self.dim * 4, k=k, groups=groups)

        # 2. 特征融合层
        # 融合维度: 64 + 64 + 128 + 128 = 384
        fusion_dim = self.dim * 19
    ...
    def forward(self, x):
        ...
        x5, idx = self.edge_conv5(x4, idx=idx)  # (B, 128, N)
        x6, idx = self.edge_conv6(x5, idx=idx)  # (B, 128, N)

        # 拼接多尺度局部特征
        local_features = torch.cat((x1, x2, x3, x4,x5,x6), dim=1)  # (B, 384, N)
