...
def read_data(file_path):
    ...
    tooth_mask = np.zeros((cfg.tooth_nums, cfg.fxid_tnums + cfg.match_tnums), np.int32)
    ...
    for key in file_data:
        ...
        tooth_mask[int(key) - 1, sorted_key_idx] = 1
        tooth_mask[int(key) - 1, match_key_idx] = 1
    ...
    teeth_landmarks = teeth_landmarks
...
class TrainData():
    ...
    def __getitem__(self, item):
        ...
        tooth_mask = np.sum(tooth_point_mask, axis=1)
        ...
        heat_map = get_hotmap(data_points, teeth_landmarks, tooth_point_mask)
        ...
        return data_points, tooth_point_mask, teeth_landmarks, heat_map

def get_hotmap(data_points, teeth_landmarks, tooth_point_mask):

    mask = np.sum(tooth_point_mask, axis=-1)
    index = np.where(mask>=1)[0]

    heat_map = np.zeros((cfg.tooth_nums, cfg.sam_points, 1), np.float32)
    for id in index:
        points = data_points[id]
        landmarks = teeth_landmarks[id]
        mask = tooth_point_mask[id] > 0

        indices, distances = find_nearest_k_points(points, landmarks[mask], k=10)
        ...
        # 4. 填充 heat_map
        heat_map[id, final_indices, 0] = gaussian_weights
