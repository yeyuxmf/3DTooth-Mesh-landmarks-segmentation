...
from model.tooth_landmark_transnet import ToothLandmark
#from model.tooth_landmarks_ainet import ToothLandmark
from model.loss import dice_loss
import config.config as cfg
from util import IOStream, accumulate_net, cal_acc
...
def train(args, io):
    ...
    file_path = "E:/DataSet/mesh_tooth_landmark_data/Teeth3DS_data_train/test/"
    ...
    # torch.jit.save(scripted_module, './save_model/tooth_landmarks.pt')
    ...
    for epoch in range(args.epochs):
        ...
        for batch_data, label_mask, label_landmarks, heat_map in train_loader:

            nums = nums +1
            batch_data = batch_data.cuda().float()
            heat_map = heat_map.cuda().float()
            label_mask = label_mask.cuda().long()
            label_landmarks = label_landmarks.cuda().float()
            mask = torch.sum(label_mask, dim=-1)

            fl_mask = label_mask[..., :5].bool()
            match_mask = label_mask[..., 5:].bool()
            fl_landmarks = label_landmarks[..., :5, :]
            ml_landmarks = label_landmarks[..., 5:, :]

            optimizer.zero_grad()
            with autocast():
                prelandmarks, matchLandmarks, heat_out, flandmarks, mLandmarks = model(batch_data, mask)
                #prelandmarks, matchLandmarks, heat_out = model(batch_data, mask)

            # noise_prelandmarks, noise_matchLandmarks, noise_heat_out = prelandmarks[1:, ...], matchLandmarks[1:, ...], heat_out[1:, ...]
            # noise_mask = mask.expand(noise_prelandmarks.shape[0], -1)
            # noise_heat_map = heat_map.expand(noise_prelandmarks.shape[0], -1, -1, -1)
            # noise_fl_landmarks = fl_landmarks.expand(noise_prelandmarks.shape[0], -1, -1, -1)
            # noise_fl_mask = fl_mask.expand(noise_prelandmarks.shape[0], -1, -1)
            # noise_ml_landmarks = ml_landmarks.expand(noise_prelandmarks.shape[0], -1, -1, -1)
            # noise_match_mask = match_mask.expand(noise_prelandmarks.shape[0], -1, -1)
            ...
            #noise_diceloss_ = dice_loss(noise_heat_out[noise_mask >= 1], noise_heat_map[noise_mask >= 1])
            #noise_f_coord_loss_, noise_f_cls_loss_, noise_fcoord_loss_, noise_floss_ = model.fixed_loss(noise_prelandmarks, noise_fl_landmarks, noise_fl_mask, epoch)
            ...
            if torch.sum(match_mask) > 0:
                ...
                md_coord_loss_,  md_cls_loss_, mdcoord_loss_, mdloss_ = model.match_loss(mLandmarks[0:1, ...], ml_landmarks, match_mask)

                #noise_m_coord_loss_, noise_m_cls_loss_, noise_mcoord_loss_, noise_mloss_ = model.match_loss(noise_matchLandmarks,noise_ml_landmarks, noise_match_mask, epoch)

                loss = (floss + mloss + diceloss_) + (mdloss_ + fdloss_)#*0.5 + (noise_floss_ + noise_mloss_ + noise_diceloss_)*0.5
            else:
                loss = (floss + diceloss_)+ (fdloss_)#*0.5 + (noise_floss_ + noise_diceloss_)*0.5
            ...
            if nums % cfg.LOSSNUMS == 0:
                ...
                f_coord_loss = f_coord_loss / cfg.LOSSNUMS
                f_cls_loss = f_cls_loss / cfg.LOSSNUMS
                m_coord_loss = m_coord_loss / cfg.LOSSNUMS
                m_cls_loss = m_cls_loss / cfg.LOSSNUMS
                fcoord_loss = fcoord_loss / cfg.LOSSNUMS
                mcoord_loss = mcoord_loss / cfg.LOSSNUMS
                diceloss = diceloss / cfg.LOSSNUMS

                fdloss = fdloss / cfg.LOSSNUMS
                mdloss = mdloss / cfg.LOSSNUMS

                noise_floss = noise_floss / cfg.LOSSNUMS
                noise_mloss = noise_mloss / cfg.LOSSNUMS
                noise_diceloss = noise_diceloss / cfg.LOSSNUMS
                ...
                print(
                    'epoch %d /%d,epoch %d /%d, total_loss: %.6f, f_coord_loss: %.6f, f_cls_loss: %.6f, m_coord_loss: %.6f, m_cls_loss: %.6f, const time: %.6f' % (
                        epoch, args.epochs, nums, inter_nums, total_loss, f_coord_loss, f_cls_loss, m_coord_loss, m_cls_loss, toc - tic))
        ...
        if epoch+1 >=0 and 0 == (epoch+1)%1:
            model.eval()
            #
            fm_cdv, all_metrics, score, fm_cdvd, all_metricsd, scoresd = cal_acc(model, test_loader)
            ...
            if ap_ar_v > max_ap_ar:
                max_ap_ar = ap_ar_v
                torch.save({'model': model.state_dict(), 'epoch': epoch}, 'outputs/' + 'best.pth')
    ...
    torch.save({'model': model.state_dict(), 'epoch': epoch}, 'outputs/final_model' + "" + '.pth')
