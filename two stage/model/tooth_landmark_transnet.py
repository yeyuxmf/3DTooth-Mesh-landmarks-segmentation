...
class ToothLandmark(nn.Module):
    ...
    def forward(self, p_points, trans_mask):
        # TB: Total Batch (Batch * Teeth_Num if merged outside) or Batch
        # TN: Teeth Num
        ...
        points = points.reshape(TB * TN, N, -1)  # Flatten for Transformer
        ...
        # concat [heatmap_feat, transformer_feat]
        x_combined = torch.cat([linearh, torch.cat(xxin[-1:], dim=-1)], dim=-1)

        # linearMap 输出的是每个点的特征 (TB*TN, N, embed_dim*4)
        # 这个特征包含语义信息，我们将其保存用于 Stage 2
        s1_point_features = self.s1_refinement(x_combined)
        #s2_point_features = self.s2_refinement(x_combined)

        # Stage 1 Prediction (Global Max Pooling)
        x_global = torch.max(s1_point_features, dim=1)[0]
        x_global = self.linearMap1(x_global)

        # Coarse Prediction (Relative to Centroid)
        predlandmarks = self.predlandmarks(x_global).reshape(TB, TN, self.fm_tnums, -1)

        # 恢复绝对坐标 (Stage 1 Output)
        ...
        # =========================================================
        # Stage 2: Local Refinement (Improved)
        # =========================================================

        # 1. 准备 Query 坐标 (Detach Gradient from Stage 1)
        # 我们希望 Stage 2 学习修正 Stage 1 的错误，而不是让 Stage 1 为了 Stage 2 能够采样到好特征而改变自己
        init_coords = predlandmarks_abs[..., :3].reshape(TB * TN, self.fm_tnums, 3).detach()
        input_pos_flat = input_pos.reshape(TB * TN, config.sam_points, 3)  # 原始绝对坐标

        # 2. KNN 搜索 (找 Coarse 预测点附近的原始点)
        idx = knn_cross(init_coords, input_pos_flat, k=self.knn_nums)

        # 3. 几何特征聚合 (使用新的函数)
        # 传入：中心点，点云坐标，点云特征(from linearMap), 索引, 位置编码器

        land_fea = get_local_feature_with_geometry(init_coords,input_pos_flat,s1_point_features, idx,self.rel_pos_encoder)
        # 4. 降维适应 Transformer
        ...
        # for blk in self.landencoder2:
        #     mland_fea = blk(mland_fea)
        ...
        # 7. Predict Offset
        # Stage 2 输出的是相对于 init_coords 的微调量 (Residual)
        land_offsets = self.outLandmarks(fland_fea, mland_fea)

        # 8. Final Coordinates = Coarse(Detached) + Offset
        # 注意: init_coords 是 detach 的，所以梯度只流过 land_offsets
        ...
        cp = cp.reshape(TB, TN, 1, -1).detach()

        init_coords = init_coords.reshape(TB, TN, self.fm_tnums, -1).detach()
