...
class ToothLandmark(nn.Module):
    ...
    def forward(self, p_points, trans_mask):
        ...
        # 鲁棒的中心化+归一化（替换原始中心化）
        p_points_normalized, cp, _ = self.center_and_normalize(p_points[..., :3])

        # 训练时添加噪声（设备无关化）
        if self.training:
            noise_scale = config.noise_scale if hasattr(config, 'noise_scale') else 1.0
            noise = torch.randn_like(cp, device=self.device) * noise_scale
            cp = cp + noise

        # Encoder Forward
        points = self.linear1(p_points_normalized)
        ...
        # 多尺度特征融合（增强核心）
        x = multi_scale_feature_fusion(teeths)
        x = self.linearB(x)
        ...
        # 恢复 Seg Score 形状以便 Loss 计算 (TB, TN, N, 1)
